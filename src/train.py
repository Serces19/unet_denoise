...
def train_fn(args):
    ...
    if args.val_split > 0:
        ...
        print("ADVERTENCIA: No se usará validación. Entrenando con todos los datos.")
        train_dataset = PairedImageDataset(input_dir=args.input_dir, gt_dir=args.gt_dir, crop_size=args.crop_size, augment=True, advanced_augment=args.advanced_augment, target_mode=args.target_mode)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        # Si existe un set de datos de validacion
            ...
            with torch.no_grad():
                for input_img, target_img in val_loader:
                    ...
                    with torch.autocast(device_type=device.type, dtype=torch.float16, enabled=(device.type == 'cuda')):
                        ...
                        loss_output = loss_fn(predicted_output, target_img)
                    if isinstance(loss_output, dict):
                        for k, v in loss_output.items(): val_loss_components.setdefault(k, 0.0)
                        val_loss_components[k] += v.item()
                    else:
                        val_loss_components.setdefault('total', 0.0)
        ...
        # Guardar mejor modelo basado en pérdida de entrenamiento
        else:
            print()
            if avg_loss < best_val_loss:
                best_val_loss = avg_loss
                model_save_path = os.path.join(args.model_output_dir, f"best_model_trainloss_{args.run_name}.pth")
                print(f"  -> Nueva mejor pérdida de entrenamiento. Guardando checkpoint en {model_save_path}")
                checkpoint = {
                    'epoch': epoch,
                    'model_state_dict': model.state_dict(),
                    'optimizer_state_dict': optimizer.state_dict(),
        ...
        if epoch == 0 or (epoch + 1) % args.viz_epochs == 0:
            print("Generando visualizaciones para TensorBoard...")
            model.eval()
            if first_train_batch is not None:
                with torch.no_grad():
                    input_img_viz, target_img_viz = first_train_batch
                    predicted_output_for_viz = model(input_img_viz.to(device))
            ...
            if val_loader is not None and last_val_batch is not None:
                with torch.no_grad():
                    input_img_viz, target_img_viz = last_val_batch
                    predicted_output_for_viz = model(input_img_viz.to(device))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--target_mode', type=str, default='RGB', choices=['RGB', 'L'])
    parser.add_argument('--n_out_channels', type=int, default=3)
    ...
    parser.add_argument('--w_laplacian', type=float, default=0.5)
    parser.add_argument('--w_ssim', type=float, default=0.25)
